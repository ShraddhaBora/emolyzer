...
export default function MetricsPage() {
    ...
    const methodItems = [
        { title: 'Preprocessing', body: 'Conjunction-aware negation marking, synonym injection for high-intensity emotion words, and bigram TF-IDF vectorisation with sublinear scaling.' },
        ...
        { title: 'Data Sources', body: '~470K samples: Twitter emotion dataset, GoEmotions (Reddit), and DAIR-AI emotion — mapped to 7 Ekman-style classes.' },
    ]
    ...
    return (
        <div className="container" style={{ paddingTop: '2rem', paddingBottom: '3rem' }}>
            <AnimatePresence mode="wait">
                ...
                {loading && <LoadingState key="loading" label="Loading model metrics…" />}
                ...
                {!loading && !error && data && (
                    ...
                    >
                        ...
                        <div className="stat-grid" style={{ marginBottom: '1.75rem' }}>
                            {[
                                { value: data.champion_model, label: 'Selected Model', d: 0.30 },
                                { value: `${(data.holdout_accuracy * 100).toFixed(1)}%`, label: 'Holdout Accuracy', d: 0.35 },
                                { value: data.macro_f1?.toFixed(3), label: 'Macro F1 Score', d: 0.40 },
                                { value: data.weighted_f1?.toFixed(3), label: 'Weighted F1 Score', d: 0.45 },
                                { value: data.test_samples?.toLocaleString?.() ?? data.test_samples, label: 'Test Samples', d: 0.50 },
                            ].map(({ value, label, d }) => (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                            <motion.div {...f(0.60)} className="card">
                                <p className="section-title">Cross-Validation Results</p>
                                <table className="metrics-table">
                                    <thead>
                                        <tr><th>Model</th><th>Mean F1</th><th>Std Dev</th></tr>
                                    </thead>
                                    <tbody>
                                        {cv.map((m, i) => (
                                            <motion.tr
                                                key={m.model}
                                                className={m.model === data.champion_model ? 'champion-row' : ''}
                                                initial={{ opacity: 0 }}
                                                animate={{ opacity: 1 }}
                                                transition={{ delay: 0.70 + i * 0.09, duration: 0.4, ease: 'easeOut' }}
                                            >
                                                <td>
                                                    {m.model}
                                ...
                                <div style={{ width: '100%', height: 240 }}>
                                    <ResponsiveContainer>
                                        <RadarChart data={radarData}>
                                            ...
                                            <Tooltip contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, fontFamily: 'Inter', fontSize: 12 }} formatter={(v) => [`${v.toFixed(2)}%`, 'Mean F1']} />
                                        </RadarChart>
                                    </ResponsiveContainer>
                                </div>
                            </motion.div>
                        ...
                        {/* Methodology */}
                        <motion.div {...f(0.58)} className="card" style={{ marginTop: '1.5rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginTop: '0.5rem' }}>
                                {methodItems.map(({ title, body }, i) => (
                                    <motion.div
                                        ...
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        transition={{ delay: 0.65 + i * 0.08, duration: 0.45, ease: 'easeOut' }}
                                    >
