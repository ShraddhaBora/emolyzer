...
def build_models(C: float = 1.0) -> dict:
    ...
    return {
        "Logistic Regression": LogisticRegression(
            C=C, max_iter=1000, solver="lbfgs", random_state=RANDOM_STATE
        ),
...
def train_and_cross_validate(
    df: pd.DataFrame, max_features: int = 30_000, C: float = 1.0
):
    """
...
def get_feature_importance(pipeline: Pipeline, top_n: int = 12) -> dict:
    ...
    if hasattr(classifier, "base_estimator") and hasattr(
    ...
    if not (hasattr(clf_model, "coef_") or hasattr(clf_model, "feature_log_prob_")):
        return {
            "Note": [("Feature interpretation not supported for this algorithm", 1.0)]
        }
...
def get_top_misclassifications(
    confusion_matrix: np.ndarray, class_names: list, top_k: int = 5
) -> list:
    """
    ...
    for token in tokens:
        ...
        if bare.startswith("not_"):
            ...
            try:
                ...
                pass
        if len(extras) > 30:
            break  # Absolute cap to prevent slowdown
...
def predict_emotion(pipeline: Pipeline, text: str) -> dict:
    ...
    probabilities = {
        EMOTION_MAP[int(c)]: float(p) for c, p in zip(class_indices, proba_array)
    }
