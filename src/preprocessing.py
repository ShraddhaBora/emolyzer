...
SCOPE_RESET_WORDS = frozenset({
    "but", "and", "or", "because", "however", "although", "though",
    "while", "until", "that", "so", "which", "when", "after", "since",
})
...
def mark_negations(text: str) -> str:
    ...
    for token in tokens:
        ...
        if _SCOPE_RESET.search(token) or clean in SCOPE_RESET_WORDS:
            ...
            base = clean if clean in ANTONYM_MAP else (squashed if squashed in ANTONYM_MAP else None)
            if base:
                result.extend(ANTONYM_MAP[base])
        else:
            result.append(token)
            # Determine which form hits the intensity map (exact or squashed)
            boost_key = clean if clean in INTENSITY_BOOST_MAP else (
                squashed if squashed in INTENSITY_BOOST_MAP else None
            )
            if boost_key:
